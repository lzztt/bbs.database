...
BEGIN
	IF LOCATE(',', $tids) = 0 THEN
		SELECT id, title, (SELECT COUNT(*) FROM comments AS c WHERE c.nid = n.id) AS comment_count 
		FROM nodes AS n 
		WHERE create_time > $time AND tid = CAST($tids AS UNSIGNED INTEGER) AND status = 1 
		ORDER BY comment_count DESC 
		LIMIT $limit;
	ELSE
		SET @sql = CONCAT('SELECT id, title, (SELECT COUNT(*) FROM comments AS c WHERE c.nid = n.id) AS comment_count FROM nodes AS n WHERE create_time > ', $time, ' AND tid IN (', $tids, ') AND status = 1 ORDER BY comment_count DESC LIMIT ', $limit);
