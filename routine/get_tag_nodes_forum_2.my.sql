DROP PROCEDURE IF EXISTS get_tag_nodes_forum_2;

DELIMITER ;;
CREATE DEFINER=web@localhost PROCEDURE get_tag_nodes_forum_2(IN $city_id INT, IN $tid INT, IN $limit INT, IN $offset INT)
    COMMENT 'get latest node for a given forum'  
BEGIN
    ...
    WHILE NOT $done DO
        ...
        ELSE
            SELECT MAX(id), COUNT(*) INTO $cid, $count FROM comments WHERE nid = $nid;
