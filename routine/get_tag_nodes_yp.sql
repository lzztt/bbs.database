...
BEGIN
    IF LOCATE(',', $tids) = 0 THEN
        ...
        (SELECT COUNT(*) - 1 FROM comments WHERE nid = n.id) AS comment_count,
        ...
        OFFSET $offset;
    ELSE
        SET @sql = CONCAT('SELECT n.id, n.title, n.view_count, yp.*, (SELECT COUNT(*) - 1 FROM comments WHERE nid = n.id) AS comment_count, (SELECT AVG(rating) FROM yp_ratings WHERE nid = n.id) AS rating_avg, (SELECT COUNT(*) FROM yp_ratings WHERE nid = n.id) AS rating_count FROM nodes AS n JOIN node_yellowpages AS yp ON yp.nid = n.id JOIN ads AS ad ON yp.ad_id = ad.id WHERE n.status > 0 AND ad.exp_time > UNIX_TIMESTAMP() AND n.tid IN (', $tids, ') ORDER BY ad.exp_time DESC, n.id DESC LIMIT ', $limit, ' OFFSET ', $offset);
