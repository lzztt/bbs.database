...
DELIMITER ;;
CREATE DEFINER=web@localhost PROCEDURE get_tag_recent_comments_yp(IN $tids VARCHAR(2000), IN $limit INT)
...
BEGIN
    IF LOCATE(',', $tids) = 0 THEN
        ...
        LIMIT $limit;
    ELSE
        SET @sql = CONCAT('SELECT c.nid, MAX(c.create_time) AS create_time, n.title, COUNT(*) - 1 AS comment_count FROM comments AS c JOIN nodes AS n ON c.nid = n.id JOIN (SELECT yp.nid, ad.exp_time FROM node_yellowpages AS yp JOIN ads AS ad ON yp.ad_id = ad.id WHERE ad.exp_time > UNIX_TIMESTAMP()) AS a ON a.nid = n.id WHERE n.tid IN (', $tids, ') AND n.status = 1 GROUP BY c.nid HAVING comment_count > 0 ORDER BY create_time DESC LIMIT ', $limit);
